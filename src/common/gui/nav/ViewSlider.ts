		...
		if (this.isModalBackgroundVisible) {
			return [
				m(".fill-absolute.will-change-alpha", {
					...
					oncreate: (vnode) => {
						this.busy.then(() => animations.add(vnode.dom as HTMLElement, alpha(AlphaEnum.BackgroundColor, theme.shadow, 0, 0.5)))
					},
					onbeforeremove: (vnode) => {
						return this.busy.then(() => animations.add(vnode.dom as HTMLElement, alpha(AlphaEnum.BackgroundColor, theme.shadow, 0.5, 0)))
					},
					onclick: () => {
