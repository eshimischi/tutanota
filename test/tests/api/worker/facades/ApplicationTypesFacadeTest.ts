...
o.spec("ApplicationTypesFacadeTest", function () {
	...
	o.beforeEach(async function () {
		...
		when(serverModelInfo.getApplicationTypesJsonUint8Array()).thenReturn(jsonContentBuffer)
	...
	o("server model should be assigned to memory first and write to file later", async () => {
		...
		when(serviceExecutor.get(ApplicationTypesService, null)).thenResolve(mockResponse)
		const jsonContentData = stringToUtf8Uint8Array("applicationTypesJsonContent")
		when(serverModelInfo.getApplicationTypesJsonUint8Array()).thenReturn(jsonContentData)
